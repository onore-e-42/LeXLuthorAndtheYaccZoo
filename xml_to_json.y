...
dedication_cnt	: TEXT {$$ = "\"content\": [\n\"" + $1 + "\"],";}
		|
		;

preface		: PREFACE_OPEN END_TAG {$$ = "{\"tag\":\"preface\"}";}
		| PREFACE_OPEN CLOSE_TAG preface_cnt PREFACE_CLOSE {$$ = "\"tag\":\"preface\"\n" + $3 + "};}
		;

preface_cnt	: TEXT	{$$ = "\"content\": [\n\"" + $1 + "\"],";}
		|
		;
/* qui secondo me è da fare con parts e part altrimenti le parti sono infinite*/
part		: PART_OPEN part_att END_TAG  {$$ = "{\"tag\":\"book\"" + $2 + "\"}";}
		| PART_OPEN CLOSE_TAG part_cnt PART_CLOSE
		| part part
		;

part_att	: ATT_ID STRING	{$$ = "\"@id\":\"" + $2 + "\",";}
			| ATT_TITLE STRING	{$$ = "\"@title\":\"" + $2 + "\",";}
			|
			;
...
toc		: TOC_OPEN END_TAG {$$ = "{\"tag\":\"toc\"}";}
		| TOC_OPEN CLOSE_TAG toc_cnt TOC_CLOSE {$$ = "{\"tag\":\"toc\"" + $3 + "}";}
		;
